...
private:
    ...
    int DownloadJson(const char* URL, const char *SerialNumber, const char *CurrentVersion, String& payload)
    {
        HTTPClient http;
        http.begin(URL);
        http.addHeader("X-Mac-Address", WiFi.macAddress());
        http.addHeader("X-Board", ARDUINO_BOARD);
        http.addHeader("X-Version", CurrentVersion);
        http.addHeader("X-Serial-Number", SerialNumber);
...
public:
    ...
    int CheckForOTAUpdate(const char* JSON_URL, const char *CurrentVersion, const char *SerialNumber, ActionType Action = UPDATE_AND_BOOT)
    {
        CurrentVersion = CurrentVersion == NULL ? "" : CurrentVersion;

        // Downloading OTA Json...
        String Payload;
        int httpResponseCode = DownloadJson(JSON_URL, SerialNumber, CurrentVersion, Payload);
        if (httpResponseCode != 200)
        ...
        for (auto config : doc["Configurations"].as<JsonArray>())
        {
            ...
            {
                Serial.println("CVersion: " + CVersion);
